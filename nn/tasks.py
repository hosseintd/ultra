...
from ultralytics.nn.modules import (ChannelAttention,CoordAtt,CSP1_X,CBS,SEBlock,C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x, Classify,
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (CoordAtt,CBS,CSP1_X,Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)

            # print(args,m,'111111111111111111111111111111')
            args = [c1, c2, *args[1:]]
            # print(args,m,'222222222222222222222222222222')
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x,CSP1_X):
                args.insert(2, n)  # number of repeats
                n = 1
                # print(args,m,'33333333333333333333333333')
