...
class CSP1_X(nn.Module):
    def __init__(self, in_channels, out_channels, n):
        ...
        self.branch1 = nn.Sequential(
            CBS(in_channels, out_channels//2),
            *[ResidualBlock(out_channels//2, out_channels//2) for _ in range(n)],
            nn.Conv2d(out_channels//2, out_channels//2, kernel_size=3, stride=1, padding=1)
        )
        self.branch2 = nn.Conv2d(in_channels, out_channels//2, kernel_size=3, stride=1, padding=1)
        self.bn = nn.BatchNorm2d(out_channels)
    ...
    def forward(self, x):
        ...
        return self.ac(self.bn(concated))
        
# CA Module Source: https://github.com/houqb/CoordAttention
